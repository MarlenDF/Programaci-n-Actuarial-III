...
En la siguiente liga, podemos encontrar una descripción de la base de datos y de su la forma en que fue creada:
http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones

Asimismo, los datos utilizados en este proyecto pueden ser descargados en la siguiente liga:
...
**README.md**: En la cual se hace una descripción de los pasos para hacer el análisis.

**CodeBook.md**: Incluye una descripción la base de datos, de las variables y las transformaciones que se realizaron a la base.

**correr_análisis.R**: Código que ejecuta el análisis.

**Completed.txt**: Base de datos ordenada con los 30 sujetos y los 66 datos.
...
2. Extrae las medidas de media y desviación estándar de cada medición.
...
`for(i in 1:SLen) {`
    `for(j in 1:activityLen) {`
        `resultado[f, 1] <- sort(unique(dataS)[, 1])[i]`
        `resultado[f, 2] <- activity[j, 2]`
        `die1 <- i == cleandata$sujeto`
        `die2 <- activity[j, 2] == cleandata$actividad`
        `resultado[f, 3:colLen] <- colMeans(cleandata[die1&die2, 3:colLen])`
        `f <- f + 1`
    `}`
`}`
